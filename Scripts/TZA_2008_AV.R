...
HH08 <- HH08 %>%
  ...
  mutate(children = ifelse(age >5 & age < 13, 1, ifelse(NA))) %>%
  mutate(young_adults = ifelse(age >12 & age < 19, 1, ifelse(NA))) %>%
  mutate(mature_adults = ifelse(age >18 & age <46, 1, ifelse(NA))) %>%
  mutate(older_adults = ifelse(age >45, 1, ifelse(NA)))

Age <-
  select(HH08,hhid, indidy1, infants, children, young_adults,mature_adults, older_adults) %>% 
  replace(is.na(.), 0) %>%
  group_by(hhid) %>% 
...
HH08 <- select(HH08, -infants, -children, -young_adults, -mature_adults, -older_adults)
...
key <- read_dta("D:/UserData/verbe038/TZA/2012/Data/NPSY3.PANEL.KEY.dta") %>%
